...
class UsersController extends JoshController
{
    ...
    public function update(User $user, UserRequest $request)
    {
        $rules = array(
            'pic_file' => 'image|mimes:jpg,jpeg,bmp,png',
            'pic_legal' => 'image|mimes:jpg,jpeg,bmp,png',
        );

        $validator = Validator::make($request->only('pic_file', 'pic_legal'), $rules);
        ...
        try {
            ...
            $user->approved = $request->get('approve');
            ...
            // is new image uploaded?
            if ($file = $request->file('pic_legal')) {
                $extension = $file->getClientOriginalExtension() ?: 'png';
                $folderName = '/uploads/users/legal/';
                $destinationPath = public_path() . $folderName;
                $safeName = str_random(10) . '.' . $extension;
                $file->move($destinationPath, $safeName);
                //delete old pic if exists
                if (File::exists(public_path() . $folderName . $user->pic_legal)) {
                    File::delete(public_path() . $folderName . $user->pic_legal);
                }

                //save new file path into db
                $user->pic_legal = $safeName;
     ...
     * @return  View
     */
    public function getModalApprove($id = null)
    {
        $model = 'users';
        $confirm_route = $error = null;
        try {
            ...
            if ($user->id === Sentinel::getUser()->id) {
                // Prepare the error message
                $error = Lang::get('users/message.error.approve');
        ...
        $confirm_route = route('approve/user', ['id' => $user->id, 'flag' => (1-$user->approved)]);
        $msg_route = 'approve';
        if ( $user->approved == 1 ) {
            $msg_route = 'cancel_approve';
        }
        return view('admin.layouts.modal_confirmation_approve', compact('error', 'model', 'confirm_route', 'msg_route'));
    }
    ...
    public function approve($id = null)
    {
        ...
        try {
            ...
            $user->approved = $flag;
            $user->save();
            $success = Lang::get('users/message.success.approved');
            if ( $flag == 0 ) {
                $success = Lang::get('users/message.success.unapprovd');
            }
            // Redirect to the user management page
            return Redirect::route('admin.users.index')->with('success', $success);
        } catch (UserNotFoundException $e) {
